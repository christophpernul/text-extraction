...
REGEX_NEW_ZEITNACHWEIS = r"\b(?:Januar|Janner|Jänner|Februar|März|Marz|April|Mai|Juni|Juli|August|September|Oktober|November|Dezember) - \d{4}\b"
REGEX_OLD_ZEITNACHWEIS = r"\b\d{2}\.\d{2}\.\d{4}\b"

# Folder Path to your PDF files
directory_root = Path('../../data')
directory_list = {
    "old": directory_root.joinpath("old"),
    "new": directory_root.joinpath("new")
}
...
def extract_year_month_old_version(text: str, regex: str) -> str:
    """Extracts pattern "dd.mm.yyyy" from a string of the old Zeitnachweis."""
...
def extract_year_month_new_version(text: str, regex: str) -> str:
    """Extracts pattern "MONTH - YEAR" from a string of the new Zeitnachweis."""
    match = re.search(regex, text)
    if match:
...
for type, directory in directory_list.items():
    for filepath in directory.iterdir():
        if filepath.suffix == '.pdf':  # Check for specific file extensions
            ...
            if type == "old":
                ...
                print(f"ERROR! Unknown file type, only '{directory_list.keys()}' are allowed!")
    ...
    print(f"Finished with {type} Zeitnachweise!")
